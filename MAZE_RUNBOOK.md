...
- `--act_halt_bias_init`: ACT halting bias 초기값 (더 음수일수록 초기 step 증가)
- `--train_repeat_factor`: epoch 내 train 샘플 반복 배수 (optimizer update 수 증가)
- `--ponder_warmup_epochs`: 초반 epoch에서 ponder penalty를 점진적으로 켜기
...
CUDA_VISIBLE_DEVICES=0 python train.py \
  ...
  --hidden_size 512 \
  --time_limit 20 \
  --act_halt_bias_init -2.0 \
  --train_repeat_factor 4 \
  --ponder_warmup_epochs 20 \
  --time_penalty 0.001 \
  --learning_rate 1e-3 \
