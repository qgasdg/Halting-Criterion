...
- 보조 실험: `Parity`, `Addition`, `StringAddition`, `Copy`, `Reverse` 토이 태스크
...
3. **토이 태스크(Parity, Addition, Copy, Reverse)에서의 일반화 패턴 점검**
   - ID/OOD 분포에서 정확도·ponder cost·스텝 수 관찰
4. **Randomized Positional Offset을 통한 길이 일반화(Length Generalization) 연구**
   - 훈련 중 매 배치마다 위치 인덱스에 랜덤 오프셋을 적용해 절대 위치 의존성 차단
   - 훈련 길이 40 → 테스트 길이 400 OOD 설정으로 Copy/Reverse 길이 일반화 검증
...
└─ tasks/
   ...
   ├─ copy.py                             # 토이 태스크: copy (입력 시퀀스 그대로 출력)
   └─ reverse.py                          # 토이 태스크: reverse (입력 시퀀스 역순 출력)
```
...
- `src/universal_transformer.py`
  ...
  - `pos_offset` 파라미터로 위치 임베딩에 랜덤 오프셋 주입 지원 (길이 일반화용)
  - OOD 시퀀스 길이(훈련 max_length 초과)도 on-the-fly 위치 신호 생성으로 자동 대응
...
## 6.3 String Addition (UT encoder-decoder)

```bash
python run.py \
  --task string_addition \
  --model_type universal_transformer \
  --ut_act \
  --max_digits 4 \
  --string_addition_max_terms 2 \
  --string_addition_test_max_digits 40 \
  --hidden_size 256 \
  --max_steps 200000 \
  --default_root_dir runs/string_addition_ut
```

## 6.4 Copy / Reverse (길이 일반화 태스크)

Copy와 Reverse는 **Randomized Positional Offset**을 활용한 길이 일반화 실험을 위한 태스크입니다.

- **Copy**: 입력 정수 시퀀스를 그대로 출력
- **Reverse**: 입력 정수 시퀀스를 역순으로 출력
- 어휘 크기 10 (0–9), 훈련 길이 `--sequence_length`(기본 5, 논문 설정은 40), 테스트는 `--sequence_test_length`(기본 sequence_length × 10)
- `--use_random_offset`으로 위치 오프셋 활성화, `--max_offset`으로 최대 오프셋 설정

```bash
# Copy — 길이 일반화 실험 (논문 설정)
python run.py \
  --task copy \
  --model_type universal_transformer \
  --sequence_length 40 \
  --sequence_test_length 400 \
  --use_random_offset \
  --max_offset 100 \
  --hidden_size 256 \
  --ut_max_hops 6 \
...
python run.py \
  ...
  --ut_max_hops 6 \
  --max_steps 200000 \
  --default_root_dir runs/reverse_ut_act
```
...
| 지표 | 설명 |
|---|---|
| `token_accuracy` | 위치별 토큰 정확도 |
| `sequence_accuracy` | 전체 시퀀스 완전 일치 비율 |
| `mean_steps` / `steps_p50` / `steps_p90` | ACT 사용 시 추론 스텝 통계 |
| `forced_halt_ratio` | 최대 hop에 도달해 강제 종료된 비율 |
...
| 인자 | 타입 | 기본값 | 필수 여부 | 설명 |
|---|---|---|---|---|
| `--task` | str | `sudoku` | 권장 명시 | `sudoku` `maze` `parity` `addition` `string_addition` `copy` `reverse` |
...
| `--sequence_test_length` | int | `sequence_length×10` | copy, reverse | OOD 테스트 시퀀스 길이 |
| `--use_random_offset` | flag | `False` | addition, string_addition, copy, reverse | 훈련 중 랜덤 위치 오프셋 활성화 |
| `--max_offset` | int | `100` | addition, string_addition, copy, reverse | 랜덤 오프셋 최대값 |
...
- 실험 스크립트
  ...
  - 토이 태스크: [`tasks/parity.py`](./tasks/parity.py), [`tasks/addition.py`](./tasks/addition.py), [`tasks/string_addition.py`](./tasks/string_addition.py), [`tasks/copy.py`](./tasks/copy.py), [`tasks/reverse.py`](./tasks/reverse.py)
