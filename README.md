...
uv run python run.py --task maze --model_type act_rnn --data_dir data/maze-30x30-hard-1k \
    --default_root_dir runs/maze_act --wandb_project halting-criterion
uv run python run.py --task maze --model_type act_rnn --data_dir data/maze-30x30-hard-1k \
    --default_root_dir runs/maze_act --wandb_offline
# W&B 로깅을 끄고 싶을 때: --no-wandb
...
- `--wandb` / `--no-wandb`: W&B 로깅 활성화/비활성화. **기본값은 활성화(`--wandb`)** 이며, 끄려면 `--no-wandb`.
...
> 플래그가 켜진 모든 실행에 `WandbLogger` 를 부착하며 (기본 ON), 모델은 PyTorch
> Lightning `self.log(...)` 표준 패턴만 사용하므로 toy/algorithmic/NLP(`bAbI`/
> `LAMBADA`/`WMT14`) 태스크 전부 W&B 로 자동 전송됩니다 (loss, accuracy,
> ponder steps, halting histogram, BLEU 등). 태스크별 추가 작업 없이 그대로
> 사용하면 됩니다 (끄려면 `--no-wandb`).
...
각 태스크의 §6.x 예제는 이미 논문 스펙을 따릅니다. 아래는 한 화면에서 비교용으로 모은 “**paper-faithful CLI cheat sheet**” 입니다 (W&B는 기본 ON, 끄려면 `--no-wandb`).
...
| `--save_every_n_epochs` | int | `1` | 선택 | epoch 기반 체크포인트 저장 주기 (sudoku/maze/babi/lambada/wmt14) |
| `--save_every_n_steps` | int | `-1` | 선택 | **`-1`=auto** (toy/algorithmic 태스크 → 50000 step, 그 외 → 0=epoch 기반 사용). `0`=명시적 비활성. `>0`=명시적 step 주기. PL 은 step/epoch 동시 활성을 금지 |
| `--log_every_n_steps` | int | `100` | 선택 | Lightning 로그/wandb 기록 주기. NaN/Inf 이상치는 cadence 무관 즉시 기록됨 |
...
| 인자 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `--wandb` / `--no-wandb` | flag | `True` (기본 ON) | W&B 로깅 활성화/비활성화. 끄려면 `--no-wandb` |
...
> **체크포인트 저장 주기 자동 분기.**
> Toy/algorithmic 태스크 (`parity / addition / string_addition / copy / reverse / logic / sort`) 는 `IterableDataset` 을 쓰므로 epoch 경계가 사실상 `max_steps` 종료 시점 1 회뿐입니다. 그래서 `run.py` 는 `--save_every_n_steps -1` (기본 = auto) 일 때 이 태스크들에 한해 **50000 step 마다 자동 저장** 으로 전환합니다 (`epoch=000-step=050000.ckpt`, `step=100000.ckpt`, ... + `last.ckpt`).
> Sudoku/maze/babi/lambada/wmt14 같은 map-style 태스크는 기존대로 `--save_every_n_epochs` (기본 1) 가 적용됩니다.
> 명시적 제어:
> - `--save_every_n_steps 5000` → step 주기 강제 (epoch 기반은 비활성)
> - `--save_every_n_steps 0` → step 기반 비활성 (auto 무시, epoch 기반만)
>
> PL 은 `every_n_train_steps` 와 `every_n_epochs` 동시 활성을 금지하므로 둘 중 하나만 적용됩니다.
