#!/usr/bin/env python
"""Copy task: output is identical to the input sequence."""
...
class CopyDataset(torch.utils.data.IterableDataset):  # type: ignore[misc]
...
class FixedCopyDataset(torch.utils.data.Dataset):  # type: ignore[misc]
    def __init__(self, sequence_length: int, size: int, seed: int):
        ...
        self.samples = [(s, s.clone()) for s in seqs]
...
class CopyModel(pl.LightningModule):
    def __init__(
        self,
        sequence_length: int,
        ...
        self.train_dataset = CopyDataset(sequence_length)
        self.val_dataset = FixedCopyDataset(sequence_length, val_size, eval_seed)
        self.test_dataset = FixedCopyDataset(test_sequence_length, test_size, eval_seed + 1)
