...
| `--save_every_n_epochs` | int | `1` | 선택 | epoch 기반 체크포인트 저장 주기 (sudoku/maze/babi) |
| `--save_every_n_steps` | int | `0` | 선택 | **0=비활성**, >0이면 step 기반 저장 (toy/algorithmic 권장 — IterableDataset 은 epoch 경계가 max_steps 종료 시점뿐이라 epoch 기반 저장 시 학습 중간 체크포인트가 남지 않음) |
...
> **주의 — IterableDataset 태스크 (parity / addition / copy / reverse / logic / sort / lambada / wmt14) 의 체크포인트 저장 주기.**
> 이 태스크들은 `IterableDataset` 을 쓰므로 `epoch` 경계가 사실상 `max_steps` 종료 시점 1 회뿐입니다. 기본값 `--save_every_n_epochs 1` 로 학습하면 중간 체크포인트가 전혀 남지 않고 `last.ckpt` + `epoch=000-step={max_steps}.ckpt` 만 생성되어, 학습이 중간에 끊기면 처음부터 다시 시작해야 합니다.
> 장기 sweep 에서는 step 기반 저장으로 전환하세요:
> ```bash
> uv run python run.py --task parity ... --save_every_n_steps 5000
> ```
> sudoku/maze/babi 같은 epoch 기반 태스크는 기존대로 `--save_every_n_epochs` 가 정상 동작합니다.
